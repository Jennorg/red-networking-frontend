  ...
  const loadProfileData = useCallback(async () => {
    ...
    try {
      ...
      const userResponse = await axios.get(
        `https://red-networking-backend.vercel.app/api/getUser/${userIdToShow}`
      );
      ...
      if (userData) {
        ...
        if (userData.links && Array.isArray(userData.links)) {
          ...
          const githubLinkItem = userData.links.find(
            (link: string) =>
              link &&
      ...
      try {
        ...
        const projectsResponse = await axios.get(
          `https://red-networking-backend.vercel.app/api/usuario_projects/${userIdToShow}`
        );
        ...
        if (userProjects.length === 0) {
          ...
          const allProjectsData =
            allProjectsResponse.data?.data?.data ||
            allProjectsResponse.data?.data ||
            allProjectsResponse.data?.proyectos ||
            allProjectsResponse.data ||
            [];

          // Filtrar proyectos del usuario objetivo
          userProjects = allProjectsData.filter(
            (project: UserProject & { authors?: string[] }) =>
        ...
        setUserProjects(
          userProjects.map((project: UserProject & { id?: string }) => ({
      ...
      const totalViews = userProjects.reduce(
        (sum: number, project: UserProject) => sum + (project.views || 0),
        0
      );
        ...
        userProjects.map(async (project) => {
          try {
            const res = await axios.get(
              `https://red-networking-backend.vercel.app/api/projects/evaluacion/promedio/${project._id}`
            );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.data.ok) {
        ...
        // Actualizar también el usuario del contexto para que el Aside cambie
        if (user) {
          setUser({
            ...user,
            name: editForm.name,
    ...
    <DashboardLayout>
      <div className="w-full p-6 space-y-6">
                    ...
                    <a
                      href={
                        userProfile.github.startsWith("http")
                          ? userProfile.github
                          : `https://github.com/${userProfile.github}`
                      }
                      target="_blank"
        ...
        <div className="flex flex-col w-full gap-6">
          {/* Columna principal: Información Personal y Proyectos */}
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        El rol no se puede editar desde aquí
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-4">
                    ...
                    {userProfile?.website && (
                      <div className="flex items-center gap-3">
                        ...
                        <a
                          href={userProfile?.website || undefined}
                          target="_blank"
                          rel="noopener noreferrer"
                ...
                {userProjects.length > 0 ? (
                  <div className="space-y-4">
                    {userProjects.map((project) => (
                      <div
                        key={project._id}
                        className="bg-[#181b22] rounded-lg p-4 border border-gray-700 relative"
                      >
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-gray-300 text-sm">
                              {projectRatings &&
                              projectRatings[project._id] !== undefined &&
                              projectRatings[project._id] !== null
