...
async function getProyecto(id: string): Promise<Proyecto | null> {
  try {
    const res = await axios.get(
      `https://red-networking-backend.vercel.app/api/projects/${id}`
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-[#181b22] rounded-lg p-4 border border-gray-700 flex flex-col gap-2 text-gray-200">
            ...
            <ul className="list-disc list-inside text-gray-300 text-sm">
              {proyecto.tools &&
              proyecto.tools.filter((t) =>
                ["React", "Chart.js", "Bootstrap", "PDFKit"].includes(t)
              ).length > 0 ? (
                proyecto.tools
                  .filter((t) =>
                    ["React", "Chart.js", "Bootstrap", "PDFKit"].includes(t)
          ...
          <div className="bg-[#181b22] rounded-lg p-4 border border-gray-700 flex flex-col gap-2 text-gray-200">
            ...
            <ul className="list-disc list-inside text-gray-300 text-sm">
              {proyecto.tools &&
              proyecto.tools.filter((t) =>
                ["Node.js", "Express.js", "MongoDB"].includes(t)
              ).length > 0 ? (
                proyecto.tools
                  .filter((t) =>
                    ["Node.js", "Express.js", "MongoDB"].includes(t)
          ...
          <div className="bg-[#181b22] rounded-lg p-4 border border-gray-700 flex flex-col gap-2 text-gray-200">
            ...
            <ul className="list-disc list-inside text-gray-300 text-sm">
              {proyecto.tools &&
              proyecto.tools.filter((t) =>
                ["Arduino", "Balanzas Inteligentes"].includes(t)
              ).length > 0 ? (
                proyecto.tools
                  .filter((t) =>
                    ["Arduino", "Balanzas Inteligentes"].includes(t)
          ...
          <ul className="space-y-2">
            {!proyecto.comments || proyecto.comments.length === 0 ? (
              ...
              proyecto.comments.map((comment, idx) => (
                <li
                  key={idx}
                  className="bg-gray-800 rounded p-3 text-sm text-gray-200 border border-gray-700"
                >
                  {comment}
                </li>
              ))
